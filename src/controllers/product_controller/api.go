...
func (controller *Controller) _getAdminNoCategory(from, to int, searchQuery string) (*models.AdminProductGet, *models.Error) {
	logger := definition.Logger
	...
	// получаем сами продукты
	if len(searchQuery) == 0 {
		products, err = controller.productRepo.GetAdminNoCategory(from, to)
	} else {
		products, err = controller.productRepo.GetAdminNoCategoryByQuery(from, to, searchQuery)
	}
	if err != nil {
		logger.Error(err, "Get products list error", layers.Database)
		return nil, errors.AdminProductGet.With(err)
